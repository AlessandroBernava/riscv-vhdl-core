...
begin
    ...
    I_TYPE               when (opcode = opc_op_imm or opcode = opc_load or opcode = opc_jalr) else
    ...
    alu_op_o <= "00" when (opcode = opc_load or opcode = opc_store or opcode = opc_auipc or opcode = opc_jal or opcode = opc_jalr or opcode = opc_lui) else
    "01"             when opcode = opc_branch else
    "10";
    alu_src_a_o <= "01" when opcode = opc_jal or opcode = opc_auipc else
    "10"                when opcode = opc_lui else
    "00";
    alu_src_b_o <= '0' when opcode = opc_op or opcode = opc_branch else   --BUG!! anche branch!!!    -- 0 rs2 (r type) , 1 imm
    '1';
